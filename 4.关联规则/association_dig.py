...
# -*- coding: utf-8 -*-
"""
Created on Sat May 25 19:32:02 2019
...
def loadDataSet():
    return [[1, 3, 4], [2, 3, 5, 6], [1, 2, 3, 5, 6], [2, 5]]
...
def rules_From_Conseq(freq_Set, H, support_Data, br1, minConf = 0.7):
    H_cur = calc_Conf(freq_Set, H, support_Data, br1, minConf)
    if H_cur:
        m = len(H_cur[0])
        if len(freq_Set) > m + 1 and len(H_cur) > 1:
            # 这个是关键
            H_next_choose = aprioriGen(H_cur, m + 1)
            rules_From_Conseq(freq_Set, H_next_choose, support_Data, br1, minConf)
...
#print(generate_Rules(L, suppData))
rules = generate_Rules(L, suppData)
